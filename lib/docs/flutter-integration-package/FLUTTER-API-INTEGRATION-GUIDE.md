...
## 🆕 Market Type Support (v3.2)

### Overview

All market data and trading tools now support an optional `market_type` parameter that enables:
- Automatic symbol conversion (e.g., `BTC-USDT` → `BTCUSDTM` for futures)
- Market-specific validation (leverage limits, etc.)
- Type-specific features and indicators

### Supported Market Types

| Type | Description | Leverage | Symbol Format |
|------|-------------|----------|---------------|
| `spot` | Direct buy/sell | No | `BTC-USDT` |
| `futures` | Perpetual contracts | 1-100x | `BTCUSDTM` |
| `margin` | Borrowed funds | 1-10x | `BTC-USDT` |
| `options` | Call/Put contracts | No | `BTC-USDT` |

### Using market_type Parameter

#### Example 1: Get Ticker with Market Type

```dart
// Spot trading
final spotTicker = await dio.post(
  ...
  data: {
    ...
    'params': {
      'exchange': 'kucoin',
      'pair': 'BTC-USDT',
      'market_type': 'spot',  // NEW
    },
  },
);

// Futures trading (automatic conversion)
final futuresTicker = await dio.post(
  '${ApiConfig.mcpServerUrl}/api/v1/mcp/tools/execute',
  data: {
    'tool': 'get_ticker',
    'params': {
      'exchange': 'kucoin',
      'pair': 'BTC-USDT',  // Will be converted to BTCUSDTM
      'market_type': 'futures',  // NEW
    },
  },
);
```

#### Example 2: Submit Order with Market Type

```dart
// Spot order (no leverage allowed)
final spotOrder = await dio.post(
  '${ApiConfig.mcpServerUrl}/api/v1/mcp/tools/execute',
  data: {
    'tool': 'submit_order',
    'params': {
...
final futuresOrder = await dio.post(
  '${ApiConfig.mcpServerUrl}/api/v1/mcp/tools/execute',
  data: {
    'tool': 'submit_order',
    'params': {
...
#### Example 3: Get Positions by Market Type

```dart
// Get only futures positions
final futuresPositions = await dio.post(
  '${ApiConfig.mcpServerUrl}/api/v1/mcp/tools/execute',
  data: {
    'tool': 'get_positions',
    'params': {
      'exchange': 'kucoin',
      'market_type': 'futures',  // NEW: Filter by type
    },
  },
);

// Response includes market_type field
// {
//   "positions": [
//     {
//       "symbol": "BTC-USDT",  // Converted to standard format
//       "market_type": "futures",
//       "side": "long",
//       "leverage": 10,
//       ...
...
#### Example 4: Technical Analysis with Market Type

```dart
// Calculate RSI for futures
final rsi = await dio.post(
  '${ApiConfig.mcpServerUrl}/api/v1/mcp/tools/execute',
  data: {
    'tool': 'calculate_rsi',
    'params': {
...
### New Market Info Tools

#### Get Supported Market Types

```dart
final marketTypes = await dio.post(
  '${ApiConfig.mcpServerUrl}/api/v1/mcp/tools/execute',
  data: {
...
//     "futures": "Futures trading - contracts with leverage and expiration",
//     ...
//   }
// }
```

#### Get Pairs by Market Type

```dart
final pairs = await dio.post(
  '${ApiConfig.mcpServerUrl}/api/v1/mcp/tools/execute',
  data: {
    'tool': 'get_pairs_by_type',
    'params': {
      'exchange': 'kucoin',
      'market_type': 'futures',
    },
...
### Tools Supporting market_type

All these tools now accept the optional `market_type` parameter:
...
**Portfolio:**
- `get_positions` (with filtering)

### Validation Rules

The backend automatically validates:

1. **Leverage for Spot**: Rejected if leverage > 1
2. **Leverage for Futures**: Must be between 1-100
3. **Funding Rate**: Only available for futures
4. **Symbol Format**: Automatically converted based on market_type
...
try {
  ...
  if (e.response?.data['error'] == 'Invalid leverage') {
    // Handle: Leverage not allowed for spot trading
  } else if (e.response?.data['error'] == 'Invalid market_type') {
    // Handle: Invalid market type specified
  }
}
```
...
- `market_type` is **optional** - existing code continues to work
- If not specified, defaults to current behavior (futures for most tools)
- New fields use `omitempty` - won't break existing parsers

### Migration Guide

**Step 1**: Add market_type to your models
```dart
class OrderRequest {
  final String exchange;
  final String pair;
  final String? marketType;  // NEW - optional
  // ... other fields
}
```

**Step 2**: Update UI to allow market type selection
```dart
enum MarketType { spot, futures, margin, options }

DropdownButton<MarketType>(
  ...
  onChanged: (value) => setState(() => selectedMarketType = value),
)
```

**Step 3**: Include market_type in API calls
```dart
final params = {
  'exchange': 'kucoin',
  'pair': pair,
  if (marketType != null) 'market_type': marketType.name,
  // ... other params
};
```
...
1. **Always specify market_type** for new features to avoid ambiguity
2. **Use standard format** (`BTC-USDT`) and let backend convert
3. **Check features** before enabling UI elements (e.g., leverage slider)
4. **Filter positions** by market_type for cleaner UI
